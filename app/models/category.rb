class Category < ApplicationRecord
  ...
  #function
  def self.json_to_relation_update(json_data)
    return false if json_data.blank?
    nodes = []
    json_data.each do |node|
      record = {title:node["title"],key:node["key"],root:false}
      category = node.has_key?("value") ? Category.find(node["value"]) : Category.new(record)
      if node["key"] == '0-0'
        record[:root] = true
      end
      if node.has_key?("children")
        children = json_to_relation_update(node["children"])
        raise Exception.new("children save error") unless category.save(record)
        children.each do |node_id|
          category.category_relationships.find_or_create_by(child_id: node_id)
        end
      else
        raise Exception.new("leaf node save error") unless category.save(record)
      end
      nodes.push(category.id)
    end
    nodes
  ...
  def self.tree_convert_loop(node,result=[])
    if node.children.blank?
      result << {value:node.id,label:node.title,title:node.title,key:node.key}
      result
    else
      tmp_arr = []
      ...
      result << {value:node.id,label:node.title,title:node.title,key:node.key,children:tmp_arr}
    end
  end
